...
/*   Updated: 2017/07/02 05:58:12 by upopee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		loop_hook(t_env *e)
{
	...
	wm = &(e->world->mod);
	km = &(e->keymod);
	km->quit == 1 ? end_session(e, NULL, EXIT_SUCCESS) : (void)e;
	km->reset == 1 ? reset_modifiers(e->world) : (void)e;
	km->next_proj == 1 ? next_proj(km, wm) : (void)e;
	km->next_color == 1 ? next_color(e->world, &(wm->col), km) : (void)e;
	km->up_marked_alt == 1 ? (wm->col.marked_alt[wm->col.curr_set] += ALT_FACTOR) : (void)e;
	km->low_marked_alt == 1 ? (wm->col.marked_alt[wm->col.curr_set] -= ALT_FACTOR) : (void)e;
	if (km->plus == 1)
	{
		ft_printf("coucou\n");
		km->zoom == 1 ? zoom(wm, Z_PSPEED) : (wm->scale.y += ALT_FACTOR);
	}
	if (km->minus == 1)
		km->zoom == 1 ? zoom(wm, Z_MSPEED) : (wm->scale.y -= ALT_FACTOR);
	if (km->up == 1)
		km->rotate == 1 ? (wm->rot_x += R_SPEED) : (wm->translate.z += T_SPEED);
	if (km->down == 1)
		km->rotate == 1 ? (wm->rot_x -= R_SPEED) : (wm->translate.z -= T_SPEED);
	if (km->left == 1)
		km->rotate == 1 ? (wm->rot_y += R_SPEED) : (wm->translate.x += T_SPEED);
	if (km->right == 1)
		km->rotate == 1 ? (wm->rot_y -= R_SPEED) : (wm->translate.x -= T_SPEED);
	refresh_window(e);
	return (0);
